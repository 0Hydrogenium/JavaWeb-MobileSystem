...
@WebFilter(servletNames = {"AddOperatorServlet"})
public class AddOperatorFilter implements Filter {
    @Override
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        for (int i=0; i < GlobalConst.INVALID_CHAR.size(); i++) {
            if (T1.contains(GlobalConst.INVALID_CHAR.get(i))) {
                servletRequest.setAttribute("message", "登录ID不能有空格");
        ...
        if (Objects.equals(T1, "")) {
            servletRequest.setAttribute("message", "登录ID不能为空");
            servletRequest.setAttribute("status", false);
            filterChain.doFilter(servletRequest, servletResponse);
            return;
        }
        else if (Objects.equals(T2, "")) {
        ...
        else if (T1.length() > GlobalConst.COL_TO_MAX_LEN.get("TOperator").get("Operator_ID")) {
            servletRequest.setAttribute("message", "登录ID长度不能超过"+String.valueOf(GlobalConst.COL_TO_MAX_LEN.get("TOperator").get("Operator_ID")));
            servletRequest.setAttribute("status", false);
            filterChain.doFilter(servletRequest, servletResponse);
            return;
        }
        ...
        if (sqlResult.getStatus()) {
            ...
            servletRequest.setAttribute("message", "提交失败");
            servletRequest.setAttribute("status", false);
        }
